...
struct hash{
  int qtde, tamanho;
  TipoElemento **itens;
  int colisoes; // novo campo para contagem de colisões
};
...
Hash* hash_criar(int tamanho){
	...
	h->itens = (TipoElemento**) malloc(sizeof(TipoElemento*)*tamanho);  // corrigido: ponteiro para ponteiro
	if (h->itens == NULL) {
		free(h);
		return NULL;
	}
	...
	h->qtde = 0;
	h->tamanho = tamanho;
	h->colisoes = 0;  // inicializa contagem
