...
bool hash_inserir(Hash *h, TipoElemento *elemento){
	...
	// Percorre lista para encontrar posição correta para inserir ordenado
	while (atual != NULL && atual->elemento->chave < elemento->chave) {
		ant = atual;
		atual = atual->prox;
	}

	// Se encontrar chave igual, não insere duplicado
	if (atual != NULL && atual->elemento->chave == elemento->chave) {
		return false;
	}

	// Incrementa colisão se inserir em bucket com lista não vazia
	if (h->itens[pos] != NULL) {
		h->colisoes++;
	}
...
bool hash_buscar_linear(Hash *h, int chave, TipoElemento **elemento){
	...
	// Busca linear ordenada: para ao ultrapassar chave
	while (atual != NULL && atual->elemento->chave <= chave) {
